...
import { User, Mail, Lock, Eye, EyeOff, Loader2, CheckCircle, ArrowLeft, ChevronDown, Search, Phone } from 'lucide-react'
...
/* ─── Sélecteur de code pays pour le téléphone ─── */
function PhoneInput({ value, onChange, country, onCountryChange }) {
  ...
  return (
    <div className="relative flex items-center bg-white border border-gray-200 rounded-xl shadow-sm focus-within:ring-2 focus-within:ring-green-500/20 focus-within:border-green-500 transition-all" ref={dropRef}>
      ...
      {/* Bouton indicatif — intégré à gauche dans le champ */}
      <button
        type="button"
      ...
      {open && (
        <div className="absolute left-0 top-full mt-1 z-50 w-72 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
          ...
          <ul className="max-h-52 overflow-y-auto">
            {filtered.length === 0 && (
              <li className="text-xs text-gray-400 text-center py-4">Aucun resultat</li>
            )}
            {filtered.map(c => (
              <li key={c.code}>
                <button
...
export default function Register() {
  const { register, socialLogin, phoneSendOTP } = useAuth()
  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        setSuccess(true)
        // Nouvel utilisateur → shop-setup, sinon dashboard direct
        setTimeout(() => navigate('/auth/shop-setup'), 700)
      } catch (err) {
    ...
    onError: () => {
      setError('Connexion Google refusée — vérifie que localhost:5173 est autorisé dans Google Cloud Console.')
    },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === 'phone') {
        ...
        const fullPhone = `${phoneCountry.dial}${phone}`
        await register({
          username,
          email,
          password,
        ...
        setSuccess(true)
        setTimeout(() => navigate('/auth/shop-setup'), 800)
      }
    } catch (err) {
        ...
        <div className="relative z-10 flex flex-col justify-end p-10 w-full">
          <div className="space-y-3 mb-6">
            <h1 className="text-3xl font-extrabold text-white leading-tight">
              Propulsez votre <span className="text-green-400">commerce</span> vers le sommet.
            </h1>
            <p className="text-base text-gray-300 max-w-lg leading-relaxed">
              Rejoignez des milliers de vendeurs qui utilisent AfriSell pour gerer leur inventaire et leur croissance.
            </p>
          </div>
          <div className="flex items-center gap-3 bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl px-4 py-3 w-fit">
            <div className="flex -space-x-2">
              {['bg-green-300', 'bg-yellow-300', 'bg-blue-300'].map((c, i) => (
                <div key={i} className={`w-7 h-7 rounded-full border-2 border-white/30 ${c}`} />
              ))}
            </div>
            <p className="text-xs font-medium text-white">+50 000 vendeurs nous font confiance</p>
          </div>
        </div>
      </section>
      ...
      <section className="flex-1 flex flex-col items-center justify-center bg-[#f8fafc] px-4 relative overflow-y-auto py-6">
        ...
        <div className="w-full max-w-100">
          ...
          <div className="flex flex-col items-center text-center mb-4">
            ...
            <p className="text-xs text-gray-500 mt-1">Demarrez votre aventure commerciale des aujourd hui.</p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 mb-3">
            ...
            <button type="button" title="Continuer avec Facebook"
              className="flex items-center justify-center p-2.5 rounded-xl bg-white border border-gray-200 opacity-50 cursor-not-allowed" disabled>
              <FacebookIcon />
            </button>
            <button type="button" title="Continuer avec Apple"
              className="flex items-center justify-center p-2.5 rounded-xl bg-white border border-gray-200 opacity-50 cursor-not-allowed" disabled>
              <AppleIcon />
            </button>
          </div>
          ...
          {/* Message erreur */}
          {error && (
            <div className="mb-3 px-4 py-2.5 bg-red-50 border border-red-200 rounded-xl text-red-600 text-sm">
              {error}
            </div>
          )}
          ...
          <form className="space-y-2.5" onSubmit={handleSubmit}>
            ...
            {mode === 'phone' && (
              <>
                <div className="px-4 py-3 bg-green-50 border border-green-200 rounded-xl text-xs text-green-700 flex items-center gap-2 mb-1">
            ...
            {mode === 'email' && (
              <>
                {/* Nom d'utilisateur */}
                <div className="space-y-1">
                  <label htmlFor="username" className="block text-xs font-semibold text-gray-700 ml-1">Nom d'utilisateur</label>
                  <div className="relative group">
                    <User size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-green-500 transition-colors" />
                    <input id="username" type="text" placeholder="jean_dupont" required value={username}
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative group">
                    ...
                    <input id="email" type="email" placeholder="jean@exemple.com" required value={email}
                ...
                <div className="space-y-1">
                  <label className="block text-xs font-semibold text-gray-700 ml-1">Telephone</label>
                  <PhoneInput
                    value={phone}
                    onChange={setPhone}
                    country={phoneCountry}
                  ...
                  {phone && (
                    <p className="text-xs text-gray-400 ml-1">
                      Numero complet : <span className="text-gray-600 font-medium">{phoneCountry.dial} {phone}</span>
                    </p>
                  )}
                </div>
                ...
                {/* Mot de passe */}
                <div className="space-y-1">
                  <label htmlFor="password" className="block text-xs font-semibold text-gray-700 ml-1">Mot de passe</label>
                  <div className="relative group">
            ...
            <div className="flex items-start gap-2 pt-0.5">
              <input id="terms" type="checkbox" checked={agreed} onChange={e => setAgreed(e.target.checked)}
                className="mt-0.5 w-4 h-4 rounded border-gray-300 text-green-600 cursor-pointer shrink-0" />
              <label htmlFor="terms" className="text-xs text-gray-500 leading-relaxed">
                J'accepte les{' '}
                <a href="#" className="text-green-600 hover:underline">Conditions</a>
                {' '}et la{' '}
                <a href="#" className="text-green-600 hover:underline">Politique de Confidentialite</a>.
              </label>
            </div>
            ...
            <Button type="submit" size="md" className="w-full justify-center" disabled={loading || success}>
              ...
              {success
                ? (mode === 'phone' ? 'Code envoyé !' : 'Bienvenue !')
                : loading
                  ? (mode === 'phone' ? 'Envoi du code...' : 'Creation...')
                  : (mode === 'phone' ? 'Recevoir le code WhatsApp' : 'Creer mon compte')
              }
            </Button>
          </form>
