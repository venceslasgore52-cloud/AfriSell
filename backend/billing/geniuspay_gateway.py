"""
GeniusPay Gateway — même flow que CinetPay, j'utilise ça en attendant la validation CinetPay.

Variables settings requises :
    GENIUSPAY_API_KEY    — clé API GeniusPay
    GENIUSPAY_SITE_ID    — identifiant du site
    GENIUSPAY_NOTIFY_URL — URL IPN
    GENIUSPAY_RETURN_URL — URL de redirection après paiement
"""
...
GENIUSPAY_PAYMENT_URL = 'https://api.geniuspay.africa/v1/payment'
GENIUSPAY_CHECK_URL   = 'https://api.geniuspay.africa/v1/payment/check'


class GeniusPayGateway:

    def __init__(self):
        self.api_key = getattr(settings, 'GENIUSPAY_API_KEY', '')
        self.site_id = getattr(settings, 'GENIUSPAY_SITE_ID', '')

    # ── Checkout ──────────────────────────────────────────────────────────────

    def create_checkout(self, payment, request=None) -> dict:
        # même pattern que CinetPay — transaction_id max 20 chars, montant en entier XOF
        ...
        payload = {
            ...
            'notify_url':            getattr(settings, 'GENIUSPAY_NOTIFY_URL', ''),
            'return_url':            getattr(settings, 'GENIUSPAY_RETURN_URL', ''),
            'channels':              'ALL',
            'lang':                  'fr',
        ...
        resp = requests.post(GENIUSPAY_PAYMENT_URL, json=payload, timeout=15)
        data = resp.json()

        if data.get('code') != '201':
            raise Exception(f'GeniusPay erreur : {data.get("message")}')
    ...
    def handle_notify(self, post_data: dict) -> bool:
        # je ne fais pas confiance au POST seul — je rappelle leur API pour confirmer
        transaction_id = post_data.get('cpm_trans_id') or post_data.get('transaction_id')
        if not transaction_id:
            return False
        status_data = self._check_status(transaction_id)
        return self._apply_status(transaction_id, status_data)

    def _check_status(self, transaction_id: str) -> dict:
        resp = requests.post(GENIUSPAY_CHECK_URL, json={
            'apikey':         self.api_key,
            'site_id':        self.site_id,
            'transaction_id': transaction_id,
        }, timeout=15)
    ...
    def _apply_status(self, transaction_id: str, data: dict) -> bool:
        ...
        gp_status = data.get('data', {}).get('status', '')
        payment.provider_response = data

        if gp_status == 'ACCEPTED':
            ...
            sub, _ = Subscription.objects.get_or_create(
                user=payment.user,
                defaults={'plan': payment.plan, 'provider': 'geniuspay'},
            )
            sub.plan       = payment.plan
            sub.status     = 'active'
            sub.start_date = timezone.now()
            # j'ajoute 30 jours pour mensuel, 365 pour annuel — le renouvellement est manuel
        ...
        elif gp_status in ('REFUSED', 'CANCELLED'):
            payment.status         = 'failed' if gp_status == 'REFUSED' else 'cancelled'
    ...
    def verify_signature(self, post_data: dict) -> bool:
        # je vérifie que le site_id correspond — même logique que CinetPay
    ...
    def create_renewal_checkout(self, subscription) -> dict:
        ...
        amount, currency = get_amount_and_currency(subscription.plan, country, 'geniuspay')
